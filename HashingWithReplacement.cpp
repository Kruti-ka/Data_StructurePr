...
void Album::accept() {
    ...
    for (i = 0; i < n; i++) 
    {
        ...
        if (h[no].track_no == -1) {
            ...
            // Collision occurred, replace the existing record
            cout << "\nCollision occurred for key " << key[i] << ". Replacing existing record.";
            cout << "\nEnter Title: ";
            cin >> h[no].title;
            cout << "\nEnter Singer: ";
            cin >> h[no].singer;
...
void Album::HTable() {
    cout << "\nTrack No\tTitle\tSinger\tComposer";
    for (i = 0; i < 20; i++) {
        if (h[i].track_no != -1) {
            cout << "\n" << h[i].track_no << "\t" << h[i].title << "\t" << h[i].singer << "\t" << h[i].composer;
        }
    }
}
